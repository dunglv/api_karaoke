$(function() {
    ...
    klr_t.each(function(i) {
        ...
        var startKara = setInterval(function() {
            ...
            var ss = Math.floor(((timeCur % 360000) % 6000) / 100);
            var ms = Math.floor(((timeCur % 360000) % 6000) % 100);
            hh = hh < 10 ? '0' + hh : hh;
            mm = mm < 10 ? '0' + mm : mm;
            ss = ss < 10 ? '0' + ss : ss;
            var curd = hh + ':' + mm + ':' + ss + '.' + ms;
            if (klr_t.eq(i).attr('data-start') === curd) {
                klr_t.eq(i).attr('data-start', curd).addClass('voiced');
                klr_t.eq(i).css({'-webkit-transition': leav_ms+'s', '-o-transition': leav_ms+'s', 'transition': leav_ms+'s' });
            }
